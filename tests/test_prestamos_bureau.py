...
def find_bureau_path(openapi):
    if not openapi:
        # fallback to a common path if local OpenAPI not available
        return '/api/bureau/consultar', 'post'
...
def test_openapi_has_bureau():
    ...
    if path is None:
        pytest.skip('OpenAPI local spec does not contain /bureau/consultar path')
